...
本指南以“从空白赛事到可交付文件”为主线，同时保留逐菜单、逐按钮、逐字段的参考说明。它覆盖当前界面可见功能、常见操作手势、四种路线类型、接力分叉、地图翻页、检查点说明、剪线与剪圈、测量、军事坐标格网、打印区域、保存兼容性、三份真实样例地图和故障排查。
...
- 正式办赛：按“底图 → 赛事设置 → 路线 → 说明表 → 检查 → 导出”的顺序工作，并在导出前执行[第 24 节](#24-交付前检查清单)。
- 找不到某个命令：查[第 2 节](#2-顶部菜单)和[第 3 节](#3-快捷工具栏)。
- 不知道鼠标、触控或快捷键怎样操作：查[第 20 节](#20-鼠标触控与快捷键完整参考)。
- 不知道某种赛制该怎样设置：查[第 12 节](#12-积分赛)、[第 13 节](#13-团队赛)、[第 14 节](#14-军事定向军体定向)和[第 15 节](#15-分叉变化路线与接力)。
...
| 名称 | 含义 | 最容易混淆的地方 |
| --- | --- | --- |
| 赛事 | 一个项目的顶层容器，包含底图、全局检查点、路线、特殊对象、打印区域和设置 | “新建赛事”会清空整个项目，不等于只添加一条路线 |
| 全局检查点 | 赛事内可被多条路线复用的点对象 | 从一条路线移除检查点，不一定删除全局对象 |
| 路线 | 参赛者需要完成的一套线路规则 | 普通、积分、团队、军体四种类型的排序和说明表规则不同 |
| 赛段 | 一条具体线路上相邻两个路线节点之间的连接 | 可设置引导范围、弯折点和手动缺口 |
| 分叉 | 普通路线中的 fork/branch 拓扑 | “所有分支”是总览，不是一条具有唯一顺序的具体线路 |
| 地图页面 | 同一条具体线路因换图、翻图而拆出的页面 | 页面边界点会同时出现在前后两页 |
| 导出区域 | PDF 页面在地图上的纸张范围 | 可分别属于所有检查点、某路线或某路线的某一页 |
| 检查点说明 | IOF/ISCD 风格说明数据及其地图表格对象 | 编辑说明数据与在地图上放置说明表是两件事 |
| 特殊对象 | 禁区、危险区、文字、线、说明表等非检查点对象 | 可只显示于指定路线，也可显示于全部路线 |
...
4. 选择“路线 > 添加路线”，名称输入“教学路线”，类型选择“普通路线”。
5. 依次点击快捷栏“起点”“检查点”“终点”，在图上放置。起点始终放在路线最前面；若已有发图点，则放在发图点之后。终点始终放在路线末端。
...
10. 先“文件 > 保存 .ocp”，再“文件 > 创建 PDF”。在 PDF 弹窗中选择当前路线、包含底图和检查点说明，然后创建。
...
3. **路线标签**：在“所有检查点”和各条路线之间切换。
4. **左侧上部面板**：说明、分叉、报告、常量。
5. **左侧下部调整区**：显示当前选中对象的属性；没有选中对象时显示赛事设置。
6. **分叉拓扑列**：普通路线存在分叉时显示可交互的分支树。
7. **地图画布**：放置、移动、选择和预览所有地图对象。
8. **地图上方信息栏**：显示赛事、路线、长度、页面和分支选择。
9. **底部状态栏**：显示操作状态、鼠标地图坐标、未保存标记、缩放和底图强度。

电脑版中，左侧面板、分叉拓扑列和地图之间的竖向分隔条可以拖动。移动端使用“路线”和“面板”下拉框切换内容。
...
加载内置 [`samples/standalone-sample.ppen`](samples/standalone-sample.ppen)，便于快速体验路线、检查点、说明表和导出功能。

**打开 .ocp/.ppen**

从本地选择并打开 O-Composer `.ocp` 或兼容 `.ppen` 文件。选择器也允许 `.xml`，但它必须是 O-Composer/Purple Pen `course-scribe-event` 结构；这里不是 IOF XML 导入入口。打开后会重置视图缩放和平移，并选择文件中的第一条路线。
...
导出兼容 `.ppen` 文件。导出时会尽量使用 `.ppen` 兼容字段，并对 O-Composer 扩展功能做降级或省略；例如团队赛会按普通路线导出，自定义常量、队号前缀、队号位数、每棒名称以及高级翻页公式等 O-Composer 专属信息不会写入兼容 `.ppen`。普通路线直接设置的翻页点会以 Purple Pen 的换图/翻图字段保存。
...
移除当前 OMAP/XMAP 或由 OCAD 转换得到的底图。不会删除检查点、路线和特殊对象；它不能清除图片/PDF 底图。
...
打开 PDF 导出弹窗。O-Composer 只生成矢量 PDF，不会回退为整页栅格 PDF。同一条具体线路的所有地图页面会按顺序合并到一个 PDF；导出多条路线或多个分支线路时，多个 PDF 会自动打包为 ZIP。
...
导出简化的 RouteGadget-style XML，包含路线名、长度和点位 X/Y；它不是包含地图与全部赛事资源的完整 RouteGadget 赛事包。

**创建 SVG 叠加层**

导出当前路线的简化 SVG。当前文件固定约 1200 px 宽、带浅色背景，并忽略底图、特殊对象、弯折、剪线和引导等多项细节；需要后处理，不能按名称假设为透明、打印保真的完整叠加层。
...
当前实现同样把视图重置为默认缩放和平移，尚未分别按路线边界或底图边界计算“适合范围”。
...
打开当前普通路线的地图页面窗口。可在普通检查点后设置换图或翻图，也可把普通检查点原位转换为独立换图点；带分叉路线还可以使用高级 Python 按具体分支、队号或棒次计算页面动作。完整说明见[第 16 节](#16-地图翻页换图与翻图)。

**独立换图点**

添加不依附于普通检查点的地图交换点。若要插入指定线路段，请先选中该线路段，再选择“添加 > 独立换图点”并在地图上放置。进入独立换图点的线路段严格按该 leg 自身的引导设置绘制，可为不引导、部分引导或全程引导，不会因为终点是换图点而自动改成全程引导；旧地图在该点结束且不画点位符号，新地图从该位置以 IOF 7.15 圆中三角续跑点开始，三角指向下一检查点。积分路线不支持独立换图点。
...
在当前普通路线的分叉面板中，根据选中的插入位置添加 fork 分叉。积分赛、团队赛和军体定向赛不能添加分叉。
...
- **类型**：普通路线、积分路线、团队赛路线或军事定向路线。

新路线初始为空，不会自动套用赛事中已有的起点或终点。进入起点、检查点或终点添加模式后，点击地图空白处会创建新对象；点击淡化显示的已有同类对象则会将它复用到当前路线。路线中的起点自动放在最前方（若有发图点则位于其后），终点自动放在末端，不会因为当前选中了中间线段而插入到线路内部。
...
删除路线不会自动删除全局检查点；之后可运行“赛事调整 > 移除未使用检查点”，但应先检查其他路线。

**复制路线**

复制当前路线，并输入新路线名称。复制后的路线复用相同全局检查点；移动这些点会影响原路线，路线级顺序、属性和变化结构则可以单独修改。
...
底层会计算检查点与赛段负荷；当前报告界面只展示检查点负荷表，赛段负荷尚未显示。需要评估赛段拥挤时应结合“赛段长度”、路线交叉引用和人工复核。
...
手机横屏时使用左侧主面板、中间地图、右侧调整面板的布局；路线标签压缩为下拉选择。存在分叉或军体格网时还可能显示拓扑列。竖屏会出现旋转手机提示。

地图上方信息栏显示当前赛事名称、路线名称、路线类型、分叉显示模式、长度、检查点数量等。若名称很长，界面会限制其宽度，避免遮挡地图和调整区。

普通路线还会显示“地图页面”下拉框，可在 **Global**（整条线路）和各个独立页面之间切换。翻页边界检查点同时出现在前后两页：前一页保留普通检查点圆，后一页显示圆中三角形的 IOF 续跑点符号；点序号始终按整条线路连续编号。
...
普通路线中，每一行对应一个起点、检查点、换图点、发图点、强制通过点或终点。积分与军体路线会按点号排序，并在 H 列显示每个检查点分值。

可编辑内容包括：

- **A/顺序**：在普通路线中随路线顺序自动更新；积分/军体通常留空。
...
点击说明表整行会同步选中地图上的路线点。点击 C–H 单元格打开符号 palette；其中有“未指定”，悬停符号约两秒可查看释义。

当当前路线为团队赛时，说明面板会提供“必打点/自由点”添加状态。该状态只影响之后新加的普通点；已有点需在调整区修改。自由点不能是起点或终点。
...
积分、团队和军体路线不能添加分叉；军体路线在这里显示专用的格网与时间窗口面板。
...
- **移动底图**：进入只移动底图的模式；画布拖动不会改变路线对象，再按按钮、Esc 或右键退出。
...
- **名称**：路线名称。
- **类型**：普通、积分、团队赛、军事定向。切换类型会改变路线排序、说明表、分值和专用面板行为；正式编排后不建议随意来回切换。
- **编号显示**：地图上控制点标号的显示方式。
  - 顺序编号
  - 代码
  - 顺序编号和代码
...
- **终点路线**：
  ...
  - 全程引导到终点
  - 导航到收拢口后沿引导线
- **从报告中隐藏**：标记路线供部分交换格式过滤。当前五类界面报告仍会列出该路线；IOF、GPX、KML 和 RouteGadget 导出会排除它，PDF“所有路线”仍使用完整路线列表。

地图翻页是独立工具，不再显示在路线的“调整”面板中。先选择路线标签，再点击快捷栏的“地图翻页”按钮，或选择“添加 > 地图翻页”，即可打开可移动的设置窗口；它只用于普通路线，且不会改变“调整”面板当前选中的对象：
...
切换到某个独立页面后打开“设置导出区域”，选择“当前路线，第 n 页”即可为该页单独设置纸张范围；未单独设置的页面会继承整条路线的导出区域。
...
- **显示于**：所有路线，或勾选任意多条指定路线。若全部取消，逻辑会恢复为所有路线。
- “显示于”中的路线名称使用单列并允许换行，长名称会完整显示。检查点说明表是例外：它属于“所有检查点”或单独一条路线。
- **文字**：显示文本。
...
该窗口是可拖动的非模态浮窗，打开时仍可操作地图。若当前正在查看某一地图页面，“当前路线”实际指“当前路线，第 n 页”。
...
如果路线包含分叉，通常会按具体变化或接力分配展开。例外是：当前明确处于“所有分支”、导出当前路线且没有地图分页时，可以生成一份所有分支合并图。带 fork 的路线在多目标 ZIP 中会使用包含队名/棒次和字母编号的层级或文件名。
...
- 图片底图会先在约 305 dpi 的页面画布中绘制，再以高质量 JPEG 资源嵌入；路线、符号与文字仍为矢量。
...
积分赛适合在规定时间内自由选择控制点并累计分值。它不是“顺序路线加上分数”，而有独立的排序与绘制规则。

### 12.1 创建与显示规则

1. 选择“路线 > 添加路线”，类型选“积分路线”。
2. 添加起点、若干普通检查点和终点。
3. 选中每个普通检查点，在说明表 H 列或调整区填写分值。
4. 选中路线，在“编号显示”中选择地图标签格式。
...
- 同一积分路线只能指定一个“前往终点的最后一个点”。
- 在另一个点启用时，原点的设置自动清除。
- 地图发放点到起点的发放线段仍可存在。
- 赛段的全程/部分引导规则和长度计算与普通路线一致。
...
- **必打点（mandatory）**：全队必须按路线顺序经过；参与顺序编号和连线，显示在说明表主序列。
- **自由点（free）**：可由团队成员分配完成；不参与路线连线，不显示顺序号，地图标签只显示点号，并在说明表“自由控制点”分区中按点号排序。
...
4. 在图上新增或复用普通检查点。
5. 选中已有路线点时，也可在调整区切换其团队赛角色。

自由点不能是起点或终点。把点在必打/自由之间切换时，该路线引用上的分值会重置为 0；若赛事使用分值，切换后必须重新核对。

### 13.2 限制与保存

- 只连接必打点；自由点不会出现在必打点之间的紫色线路中。
- 不支持新建分叉、接力分配或地图分页。
- 导出原生 `.ppen` 时会降级为普通路线，自由点角色丢失。保留团队赛语义必须使用 `.ocp`。
...
1. 选择“路线 > 添加路线”，类型选择“军事定向”。
2. 添加起点、普通积分点和终点，并为普通点设置分值。
3. 打开左侧“分叉”面板；对军事定向路线，此处会变为专用面板。
4. 新建或选择一个坐标格网。
5. 绘制格网裁切边界。
...
一个赛事可保存多个命名格网，多条军体路线可共享同一格网。每条军体路线通过格网选择框引用其中一个。
...
> [!WARNING]
> 格网是赛事级共享对象。删除一个格网会同时清除所有引用它的军体路线的格网选择。删除前应先确认没有其他路线复用。

### 14.4 时间窗口点

时间窗口不是新的全局检查点类型，而是某条军体路线对普通检查点的一种引用属性。因此，同一个全局检查点可在军体路线 A 中是时间窗口点，在路线 B 中仍是普通积分点。
...
必须先为该军体路线选择一个有有效边界的格网，才能添加时间窗口点。

添加窗口时可以：

- 点击已有全局普通检查点进行吸附；
- 把本路线已有普通点转换为时间窗口引用；
- 复用尚未加入本路线的全局普通点，通常插到终点之前；
- 在空白位置创建新的普通全局检查点。
...
- 格网只在引用它的军体路线中显示，不显示在“所有检查点”或其他路线中。
- 从军体路线删除时间窗口，只删除本路线引用；底层全局普通检查点仍保留。

军体赛必须以 `.ocp` 作为权威工程格式。原生 `.ppen` 会把军体路线降级为积分路线，并丢失格网、边界、时间窗口、窗口排序和指定终点来源。

## 15. 分叉、变化路线与接力

接力赛不是第五种路线类型。它是一条普通路线，加上 fork/loop 变化结构、棒次限制和队伍自动分配。

### 15.1 创建前提与基本步骤

1. 新建普通路线，添加起点和分叉前公共点。
...
- 点击分支内部线段：只插入该分支。
- 点击汇合后的线段：插入所有分支汇合之后。
- 地图上选中路线点时，拓扑树同步定位对应节点。

可在路线末端建立“开放分叉”，之后分别添加分支内容；再添加一个公共点时，它会成为所有分支的汇合点。删除分支后若只剩一个分支，结构会自动折叠回普通线路。
...
- **默认/第一条具体路线**：有唯一顺序，可计算单一长度和分页。
- **所有分支**：显示整个拓扑，没有唯一经过顺序；长度显示最短–最长范围，通常不能分页。
- **具体字母编号**：显示一条确定的变化路线。
- **接力队号 + 棒次**：显示自动分配给该队该棒的具体路线。
...
- PDF 可按具体变化或接力队棒生成；同一具体路线的多张地图页合并成一个 PDF，多目标打包为 ZIP。
- 如果明确选择“所有分支”且没有分页设置，也可以输出一张所有分支总览图。
- IOF XML 会展开变化；GPX、KML、RouteGadget 和选中路线 SVG 通常只导出默认/第一分支，不能代表完整接力网络。

## 16. 地图翻页、换图与翻图

分页只适用于有唯一经过顺序的普通路线。积分、团队、军体和“所有分支”视图不能正常分页。
...
> [!WARNING]
> 把普通点转换为独立换图点是全局改变。所有复用这个全局检查点的路线都会受到影响；删除该独立换图动作时也会全局恢复为普通点。操作前应查看“检查点交叉引用”报告。
...
无分叉路线填入非空高级代码时会清除逐点翻图/换图动作；反过来修改简单动作也会清除高级代码。从旧文件导入、同时存在固定动作和高级数据时，界面会给出兼容提示，应明确选择一种权威规则。
...
| `course.course_control` | 路线点引用 ID 数组 |
...
| `course.course_name` / `course.course_id` | 路线名称与 ID |
| `course.team` / `course.leg` | 接力队号与棒次；非接力为 0 |

未受棒次限制的 `point_allowed_legs`/`allowed_legs` 项通常是空数组，表示不限制，不是“没有可用棒次”。

运行环境是项目内置 Pyodide 314.0.2（CPython 3.14 WebAssembly），在独立 Worker 中执行，每条具体路线限时约 3 秒。它不暴露编辑器 DOM，也不会按脚本自动下载第三方包。语法错误、运行错误、超时、返回长度错误或同一点冲突都会阻止 PDF 导出。数据预览最多列出前 12 条具体变化；窗口中可复制示例脚本和 AI 提示词。

“使用示例 Python”可填入模板，“查看提示词”展示面向 AI 的完整上下文，“复制 AI 提示词”复制到剪贴板。AI 生成结果仍必须在路线预览、具体变化和 PDF 中人工验证。
...
- 切换到具体页面后打开“设置导出区域”，可为“当前路线，第 n 页”单独设定纸张范围；未设置的页面继承整条路线范围。
- 导出时，同一具体路线的全部页面按顺序合并为一个 PDF。
...
OMAP/OCAD 会读取源比例尺，并同步赛事比例尺以及仍沿用旧值的路线打印比例尺。之后只在底图信息中修改数值不会重新解析或几何缩放 OMAP 对象；若源图比例尺错误，最好在制图软件中修正后重新导入。
...
编辑宽度会按原图宽高比联动高度，编辑高度会反向联动宽度；印刷宽度、实地宽度和比例尺也会互相换算。校准改变底图尺寸与比例尺信息，不改变路线符号的规定纸面尺寸。
...
1. 打开“设置 > 底图信息”。
2. 点击“移动底图”。
3. 在画布上拖动；此时只移动底图，不移动路线、检查点或特殊对象。
...
| 文件 | 实测特征 | 推荐练习 |
| --- | --- | --- |
| [`beihang_xyl_jiaoxuequ_1000.omap`](samples/beihang_xyl_jiaoxuequ_1000.omap) | 未压缩 OMAP v9，1:1000，ISSprOM 2019，217 个顶层符号、1092 个活动对象 | 矢量导入、普通路线、剪线/剪圈、背景移动、矢量 PDF |
...
### 19.2 北航 OMAP：普通路线、剪线与剪圈

1. 选择“文件 > 导入 OMAP 地图”，打开北航 OMAP 样例。
2. 状态完成后打开“设置 > 底图信息”，确认比例尺为 1:1000、对象约 1092。它不应进入图片式强制校准。
3. 在“赛事调整”中选择 ISSprOM 2019，并设置检查点说明标准。
4. 新建普通路线“北航教学区短线”。添加发图点、起点、6–10 个检查点和终点。
5. 复用测试：新建第二条路线，在“检查点”工具中点击淡化的已有圆，确认是复用全局点而非生成重复代码。
6. 选择第一条路线，点击一个普通点的圆心、圈内和圆周，确认都可选中。
...
11. 设置 A4 横向导出区域，添加说明表并导出 PDF。OMAP 与路线应保持矢量。
...
10. 保存完整 `.ocp`，再试导出原生 `.ppen`，理解其会降级为积分路线；不要把 `.ppen` 作为该练习的权威文件。
...
“整条路线”和“整张地图”当前都只是重置缩放和平移；若要精确取景，使用滚轮、拖动和导出区域框。
...
- **发图点**：始终在线路最前。
- **起点**：始终是结构端点，位于发图点之后；单条路线只能有一个，不能被残留插入锚点放到中间。
- **普通点**：通常插入当前拓扑位置；固定路线未明确选择时插到终点之前。
- **终点**：默认位于末尾；在未闭合分叉中可作为拓扑终点闭合结构；单条路线只能有一个。
- **独立换图点**：插入选中赛段；积分和军体路线不允许。
...
圆缺口保存在全局检查点上，而不是某一条路线引用上；复用该点的其他路线会显示同一缺口。剪之前应检查该点在其他路线中的连线方向是否仍合适。
...
普通特殊对象的“显示于”可选全部路线或多条具体路线。说明表不同：每个显示目标只允许一张，再次添加会选中已有表。
...
数值会自动换算：实地长度在 m/km 间切换，面积在 m²/ha 间切换；纸面值用 mm 和 mm²。测量保存在 `.ocp` 和本地会话中，不写入原生 `.ppen`，也不是正式路线对象。
...
| 能力 | `.ocp` | 原生 `.ppen` |
| --- | --- | --- |
| 普通路线、检查点、赛段、分叉 | 完整 | 通常保留 |
| 赛段弯折、剪线、控制圈缺口 | 完整 | 使用兼容字段保留 |
| 简单换图/翻图 | 完整 | 通常保留 |
| 高级 Python/旧高级分页数据 | 完整 | 丢失；有 Python 时导出前警告 |
| 团队赛与自由点角色 | 完整 | 降级普通路线，角色丢失 |
| 军体格网与时间窗口 | 完整 | 降级积分路线，扩展丢失 |
| 积分/军体指定终点来源 | 完整 | 丢失 |
| 自定义常量 | 完整 | 丢失 |
| 接力前缀、位数、棒名 | 完整 | 丢失 |
| 测量 | 完整 | 丢失 |
...
| IOF XML 3.0 | 赛事名、比例尺/范围、点、路线、长度、爬升、积分 | 每条具体变化为独立 Course | 无 | 不含底图、特殊对象或完整说明；坐标仍是内部值 |
| IOF XML 2.0.3 | 控制点、路线、CourseVariation | 路线内展开变化 | 无 | 不进行真实地理转换 |
| GPX | 全局点 waypoint、路线 route | 仅默认/第一分支 | 无 | 内部 X/Y 直接充当 lon/lat 占位，不能用于 GPS 导航 |
| KML | 路线文件夹、点和端点连线 | 仅默认/第一分支 | 无 | 忽略弯折、剪线和引导范围；坐标不是可靠经纬度 |
| RouteGadget XML | 路线名、长度、点的 X/Y | 仅默认/第一分支 | 无 | 简化的 RouteGadget-style XML，不是完整赛事包 |
| SVG | 基础路线符号 | 仅默认/第一分支 | 无 | 固定约 1200 px 宽且带浅色背景；忽略许多编辑细节，不是真正透明打印叠加层 |
...
在文字中输入常量名即可替换。例如 `\event - \course - 1:\mapscale` 会随赛事、路线和比例尺变化。
...
| 报告 | 主要列 | 用途与边界 |
| --- | --- | --- |
| 路线摘要 | 路线、类型、普通点数、长度、爬升、负荷 | 快速核对全部路线 |
| 赛事检查 | 严重级别、对象、消息 | 检查缺起终点、缺/重代码、未使用点等基础问题；不覆盖全部高级规则 |
| 赛段长度 | 路线、起点、终点、长度 | 核对技术难度与引导长度 |
| 检查点交叉引用 | 点号、类型、使用路线 | 删除或转换共享点前必须查看 |
| 检查点和赛段负荷 | 当前界面主要显示检查点负荷 | 底层虽计算赛段，UI 尚未展示赛段表 |

“路线分叉报告”列出变化代码、点序列和接力分配。赛事检查通过并不代表高级 Python、棒次限制、军体格网、时间窗口和打印比例尺全部正确，这些仍需专门检查。

### 22.5 赛事与路线属性速查

赛事级：赛事名称、控制圈尺寸百分比、ISCD 2024/2004、ISOM 2017/ISSprOM 2019/ISOM 2000、自动编号起始值、避免可倒读代码、批量移动、移除未使用点。

路线级：名称、四种类型、标签样式、打印比例尺、爬升、负荷、长度覆盖、起始编号、副标题、终点引导方式、隐藏标记，以及接力队数/棒次/首队号/显示与分配字段。

检查点级：类型、代码、X/Y、旋转、说明前后文字、团队角色、积分终点来源、C–H 说明与打卡格。

赛段级：起终点、引导类型与比例、手动缺口、弯折点。特殊对象级：显示路线集合、颜色、边界、线型、宽度、虚线长度/间距、圆角、文字字体与样式等。
...
**为什么起点跑到路线中间？** 当前规则会把起点强制放在最前，发图点存在时放在其后。若打开的是旧文件，先查看拓扑和点类型；删除错误路线引用后重新用“起点”工具复用全局起点。单条路线不能有两个起点。

**为什么点击后复用了旧点？** 添加工具会吸附附近全局点，目的是多路线共享。若要新点，远离旧点放置，再移动到准确位置，并检查代码唯一。

**删除检查点会发生什么？** 单路线视图中通常只移除当前路线引用；“所有检查点”中删除会提示它被哪些路线使用，并可从所有路线删除。删除共享点前查看交叉引用报告并保存。

### 23.3 剪圈、剪线与赛段

**无法剪 control 圈：** 确认使用“剪断连线”，目标是普通检查点或终点，并点在可见圆周而非中心。起点不能剪圈。放大后点击最容易成功。

**找不到已有圆缺口：** 在缺口弧段的中点附近点击；选中后显示两个蓝色圆周端点。若仍找不到，切换到具体路线并确认该全局点确实有缺口。
...
**分叉按钮不出现：** 路线必须是普通类型，并先在分叉面板选择每队人数/棒次数。积分、团队、军体不支持新建分叉。

**所有分支不能选择页面：** 所有分支没有唯一顺序。切到具体变化代码或接力队号+棒次。

**高级 Python 阻止 PDF：** 查看错误对应的具体路线，检查函数名、返回两个列表、列表长度等于 `course.length`、同一点没有同时翻图和换图，并避免超过 3 秒。

**创建不了时间窗口点：** 先为军体路线创建并选择一个有有效多边形边界的格网；时间必须是 `MM:ss` 且秒为 00–59。

**删格网后其他路线也没了：** 格网是赛事级共享资源，删除会清除所有路线引用。用撤销恢复，或重建并重新选择。
...
| 打印比例尺错误 | 核对校准与路线打印比例尺；PDF 阅读器必须“实际大小/100%” |
| GPX/KML 跑到错误国家 | 内部 X/Y 被当作占位 lon/lat，并非真实经纬度，不能导航 |
| SVG 不是透明叠加 | 当前 SVG 带浅色背景且只含基础路线；正式叠加优先用 PDF 或后处理 SVG |
...
- 过多套环、分叉组合和接力队棒会快速增加具体路线与 PDF 数量。
...
- [ ] 运行路线摘要、赛事检查、赛段长度、交叉引用与负荷报告，并人工覆盖高级规则。
...
最终交付后，将最后 `.ocp`、最终 PDF、导出的交换文件、原始底图和审核记录放在同一冻结版本目录。赛前若必须改点，应提高版本号，重新执行受影响路线、说明、分叉、页面和打印检查，不要只替换单张 PDF 而不保留对应工程。
...
- “整条路线”和“整张地图”当前均重置视图，尚未按不同边界自动适配。
- “从报告中隐藏”目前不会从五类界面报告隐藏路线，但会影响若干数据导出。
