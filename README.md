...
The preferred path uses the bundled OpenOrienteering Mapper WebAssembly engine in `assets/mapper-wasm/`. Its JavaScript, WebAssembly, and projection-data files total about 26 MiB, so the first visit may need time to download them in the background. The resource indicator at the top right combines those files with the app fonts and symbol data; after every byte is downloaded it switches to an indeterminate initialization state until Mapper is ready. If **Import OCAD Map** is clicked before the engine is ready, O-Composer reports the current loading state without opening a file picker or starting another loader; wait for loading to finish, then click the command again. Later visits can reuse the versioned browser cache.
